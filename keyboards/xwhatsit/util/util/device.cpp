    ...
    mutex(mutex)
{
    ...
    qDebug() << "DEVICE: before mutex";
    QMutexLocker locker(&mutex);
    qDebug() << "DEVICE: mutex acquired";
    ...
    qDebug() << "DEVICE: before hid_open_path";
    device = hid_open_path(real_path.c_str());
    qDebug() << "DEVICE: after hid_open_path" << (device != nullptr);
    if (!device)
    {
...
std::vector<uint16_t> Device::getSignalValue(uint8_t col, uint8_t row)
{
    ...
    if (!got_util_response)
    {
        throw std::runtime_error("hid_read failed while getting signal value -- no util response");
    }
    if (data[2] != UTIL_COMM_RESPONSE_OK)
    {
