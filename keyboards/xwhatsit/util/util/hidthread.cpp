...
void HidThread::nameTranslation(std::string &name)
{
    ...
    const std::string vial_prefix = "keyboards/xwhatsit/vial/";
    const std::string ibm_prefix  = "keyboards/xwhatsit/ibm/";
    if (name.rfind(vial_prefix, 0) == 0)
        name = ibm_prefix + name.substr(vial_prefix.size());
...
void HidThread::run()
{
    ...
    forever {
        ...
        {
            try {
                qDebug() << "SIGLEVEL: opening device";
                QScopedPointer<Device> dev(comm.open(l_signal_level_path));
                qDebug() << "SIGLEVEL: device opened";
                dev.data()->assertVersionIsAtLeast(2, 0, 2);
                qDebug() << "SIGLEVEL: version OK";
                std::string name = dev.data()->getKeyboardFilename();
                qDebug() << "SIGLEVEL: raw name" << QString::fromStdString(name);
                nameTranslation(name);
                qDebug() << "SIGLEVEL: translated name" << QString::fromStdString(name);
                emit keyboardName(name);
                qDebug() << "SIGLEVEL: emitted keyboardName";
                dev.data()->disableKeyboard();
                std::vector<uint8_t> details = dev.data()->getKeyboardDetails();
                cols = details[0];
                rows = details[1];
