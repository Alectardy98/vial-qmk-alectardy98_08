...
bool matrix_scan_custom(matrix_row_t current_matrix[]) {
    ...
    // Read original 18 rows x 8 bits (cols)
    uint8_t temp_matrix[18];
    for (uint8_t row = 0; row < 18; row++) {
        temp_matrix[row] = read_bits(8);
    }
    ...
    // Transpose the matrix: 18 rows x 8 bits → 8 rows x 18 bits
    bool changed = false;
    for (uint8_t col = 0; col < 8; col++) {
        matrix_row_t new_row = 0;
        for (uint8_t row = 0; row < 18; row++) {
            if (temp_matrix[row] & (1 << col)) {
                new_row |= (1UL << row);
            }
        }

        if (current_matrix[col] != new_row) {
            current_matrix[col] = new_row;
            changed = true;
        }
    }
