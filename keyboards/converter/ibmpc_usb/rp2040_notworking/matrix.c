...
uint8_t matrix_scan(void)
{
    ...
    if (ibmpc_error) {
        xprintf("\n%u ERR:%02X ISR:0x%04X", timer_read(), ibmpc_error, ibmpc_isr_debug);

        // If it's a receive error (not send/full), reset the keyboard
...
void matrix_print(void) {
    ...
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
        uint32_t bits = 0;
#if (MATRIX_COLS <= 8)
        bits = bitrev(matrix_get_row(row));
        char row_bits[9];
        for (int i = 0; i < 8; ++i) {
            row_bits[i] = (bits & (1 << (7 - i))) ? '1' : '0';
        }
        row_bits[8] = '\0';
#elif (MATRIX_COLS <= 16)
        bits = bitrev16(matrix_get_row(row));
        char row_bits[17];
        for (int i = 0; i < 16; ++i) {
            row_bits[i] = (bits & (1 << (15 - i))) ? '1' : '0';
        }
        row_bits[16] = '\0';
#elif (MATRIX_COLS <= 32)
        bits = bitrev32(matrix_get_row(row));
        char row_bits[33];
        for (int i = 0; i < 32; ++i) {
            row_bits[i] = (bits & (1UL << (31 - i))) ? '1' : '0';
        }
        row_bits[32] = '\0';
#endif
