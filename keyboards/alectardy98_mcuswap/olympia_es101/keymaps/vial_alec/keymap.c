...
enum blender_keycode {
    ...
    // New user keycodes
    V100,                   // Hold key, sends volume up 10 times quickly (one-shot on press)
    V050,                   // Hold key, sends volume down 5 times quickly (one-shot on press)
    MUT1,                   // Hold-to-mute: mute on press, unmute on release
    SOLH,                   // Hold solenoid on while held, off on release
};

// New helpers/state (added, nothing removed)
static bool is_muted = false;

void tap_volume(uint16_t kc, uint8_t times) {
    for (uint8_t i = 0; i < times; i++) {
        tap_code(kc);
        wait_ms(5); // small delay so OS registers each press reliably
    }
}
    
bool process_record_user(uint16_t keycode, keyrecord_t *record) {

    // New press+release behaviors (added, existing code below unchanged)
    switch (keycode) {
        case SOLH:
    ...
    if (record->event.pressed) {             // Code for macros
        switch (keycode) {
            ...
            // New volume keycodes (added)
            case V100:
                tap_volume(KC_VOLU, 10);
                return false;

            case V050:
                tap_volume(KC_VOLD, 5);
                return false;
        }
    }
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_BASE] = LAYOUT(
     KC_ESC,      KC_GRV,    KC_1,    KC_2,    KC_3,    KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0, KC_MINS,  KC_EQL, _______, KC_BSPC,       EXPL, SNIP,
       TASK,      KC_TAB,    KC_Q,    KC_W,    KC_E,    KC_R,    KC_T,    KC_Y,    KC_U,    KC_I,    KC_O,    KC_P, KC_LBRC, KC_RBRC,                      DM_PLY1, V050,
    KC_LCTL,     KC_LSFT,    KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L, KC_SCLN, KC_QUOT, _______, KC_ENT,                 COPY, MUT1,
    MO(_FN),              _______,    KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M, KC_COMM,  KC_DOT, KC_SLSH,                                  PAST, _______, _______, _______,
                                            KC_LALT,                    KC_SPC,                   KC_LGUI,                                                    VDLT, VDRT
    ),

    [_FN] = LAYOUT(
    QK_BOOT,     QK_BOOT,   KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,   KC_F6,   KC_F7,   KC_F8,   KC_F9,  KC_F10,  KC_F11,  KC_F12, _______,  KC_DEL,    _______, _______,
    _______,        SNIP, _______, _______, _______, _______, _______, _______, _______, KC_PAUS, KC_SCRL, KC_PSCR,   KC_UP, KC_BSLS,                      DM_REC1, _______,
    _______,     KC_LCTL, C(KC_A), C(KC_S), C(KC_D), C(KC_F), _______, _______, _______, _______, _______, KC_HOME, KC_PGUP, KC_LEFT, KC_RGHT,             _______, _______,
    _______,              _______, C(KC_Z), C(KC_X), C(KC_C), C(KC_V), _______, _______, _______,  KC_END, KC_PGDN, KC_DOWN,                               _______, _______, _______, _______,
                                            _______,                   HF_TOGG,                   _______,                                                    VDUP, VDDN
    ),
};
