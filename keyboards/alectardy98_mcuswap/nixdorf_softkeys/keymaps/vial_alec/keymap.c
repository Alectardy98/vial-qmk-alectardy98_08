...
// Track NIX key states
static bool nix1_held = false;
static bool nix2_held = false;
static bool nix3_held = false;
static bool nix4_held = false;
static bool nix5_held = false;


bool led_update_user(led_t led_state) {
    // CAPS LOCK controls GP16
    if (led_state.caps_lock) {
        writePinHigh(16);  // GP16 ON
    } else {
        writePinLow(16);   // GP16 OFF
    }

    // FN layer or SCROLL LOCK controls GP29
    if (IS_LAYER_ON(_FN) || led_state.scroll_lock) {
        writePinHigh(29);  // GP29 ON
    } else {
        writePinLow(29);   // GP29 OFF
    }
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {             // Code for macros
        switch (keycode) {
            ...
            case NIX1:
                ...
                SEND_STRING("Key 1 Engaged" SS_TAP(X_ENTER));
                break;
            case NIX2:
                nix2_held = true;
                setPinOutput(29);
                ...
                SEND_STRING("Key 2 Engaged" SS_TAP(X_ENTER));
                break;
            case NIX3:
                nix3_held = true;
                setPinOutput(29);
                ...
                SEND_STRING("Key 3 Engaged" SS_TAP(X_ENTER));
                break;
            case NIX4:
                nix4_held = true;
                setPinOutput(29);
                ...
                SEND_STRING("Key 4 Engaged" SS_TAP(X_ENTER));
                break;
            case NIX5:
                nix5_held = true;
                setPinOutput(29);
                ...
                SEND_STRING("Key 5 Engaged" SS_TAP(X_ENTER));
                break;
            case LNCH:
                setPinOutput(16);
                setPinOutput(29);
                ...
                SEND_STRING("Secure Overide Preformed, Launch Initiated in T - 5... 4... 3... 2... 1..." SS_TAP(X_ENTER));
                break;
        }
    } else {
        switch (keycode) {
    ...
    if (nix1_held && nix2_held && nix3_held && nix4_held && nix5_held) {
        layer_on(_NUKE);
    } else {
        layer_off(_NUKE);
    }
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_NUKE] = LAYOUT(
       LNCH, _______, _______, _______, _______, _______,
